...
const Formulario = ({ handleSubmit, messageError, options, setOptions, deleteOption, correctAnswer, setCorrectAnswer, loading, problem, setProblem, errorOptions, ecuacion, setEcuacion }) => {
    ...
    return (
        <div className='flex justify-center items-center'>
            <form className='border-2 rounded-md shadow-md w-1/2 my-10 py-5 px-4 bg-white' onSubmit={handleSubmit}>
                <div className='flex flex-col'>
                    ...
                    />
                    {problem.error ? (
                        <p className='p-2 bg-red-600 text-white text-center rounded mt-3'>{problem.error}</p>
                    ) : null}
                </div>
                ...
                {messageError ? (
                    <div>
                        <p className='text-sm bg-red-600 text-white text-center p-2 rounded mt-4'>{messageError}</p>
                    </div>
                ) : null}
                <div className="flex flex-col mt-4">
                    <ol>
                ...
                {options.length >= 2 ? (
                    <>
                        <div className='shadow-lg border-b my-2'></div>
                        <div>
                            <label htmlFor="correct" className='font-bold mb-1'>Selecciona la respuesta correcta</label>
                            <select
                                name="correct"
                                id="correct"
                                className='border-2 py-2 px-1 mt-2 rounded-md outline-none focus:border-blue-500 w-full'
                                value={correctAnswer.value}
                                onChange={(e) => setCorrectAnswer({ value: e.target.value, error: '' })}
                            >
                            ...
                            {correctAnswer.error ? (
                                <p className='p-2 bg-red-600 text-white text-center rounded mt-3'>{correctAnswer.error}</p>
                            ) : null}
                        </div>
                        {loading && (
                            <div>
                        ...
                        {errorOptions && (
                            <p className='p-2 bg-red-600 text-white text-center rounded mt-3'>{errorOptions}</p>
                        )}
